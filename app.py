...
# Import QGIS checker functions
from check_qgis_algorithms import get_qgis_info, get_qgis_path

# QGIS installation path - now uses environment-aware function
            ...
            for x in range(min_x, max_x + 1):
                for y in range(min_y, max_y + 1):
                    try:
                        ...
                        if tile_count % 10 == 0:  # Reduced logging frequency
...
def check_qgis():
    ...
    try:
        # Use the utility function from check_qgis_algorithms.py
        qgis_info = get_qgis_info()
        
        if not qgis_info["qgis_found"]:
            return jsonify({
                "qgis_status": "QGIS not found",
                "platform": qgis_info["platform"],
                "manual_method_available": True,
                "tile_algorithms": [],
                "message": "Using manual tile download method only",
                "suggestions": [
                    "Install QGIS Desktop for local development",
                    "Use Docker with QGIS for Railway deployment",
                    "Manual tile download is already working"
                ]
            }), 200
        
        if not qgis_info.get("qgis_working", False):
            return jsonify({
                "qgis_status": "QGIS found but not working",
                "qgis_path": qgis_info["qgis_path"],
                "platform": qgis_info["platform"],
                "error": qgis_info.get("error", "Unknown error"),
                "manual_method_available": True,
                "tile_algorithms": []
            }), 200
        ...
        return jsonify({
            ...
            "qgis_path": qgis_info["qgis_path"],
            "platform": qgis_info["platform"],
            "total_algorithms": qgis_info.get("total_algorithms", 0),
            "tile_algorithms": qgis_info.get("tile_algorithms", []),
            "manual_method_available": True,
            "message": "Both QGIS and manual methods available"
        }), 200
        
    except Exception as e:
        return jsonify({
            "qgis_status": "Error checking QGIS",
            "error": str(e),
            "manual_method_available": True,
            "tile_algorithms": [],
            "message": "Fallback to manual method"
        }), 500

@app.route('/qgis_algorithms', methods=['GET'])
def list_qgis_algorithms():
    """Development endpoint to list all QGIS algorithms"""
    
    if not QGIS_PATH:
        return jsonify({
            "error": "QGIS not available",
            "algorithms": [],
            "message": "QGIS not found on this system"
        }), 404
    ...
    try:
        ...
        if result.returncode == 0:
            algorithms = [line.strip() for line in result.stdout.split('\n') 
                         if line.strip()]
            
            return jsonify({
                "total_algorithms": len(algorithms),
                "algorithms": algorithms[:50],  # Limit to first 50
                "tile_algorithms": [alg for alg in algorithms 
                                  if 'tile' in alg.lower()][:10],
                "qgis_path": QGIS_PATH
            })
        else:
            return jsonify({
                "error": "QGIS command failed",
...
def download_mbtiles():
    try:
        ...
        max_zoom = int(data.get("max_zoom", 16))  # Reduced default max for Railway
        ...
        # Validate zoom levels (more restrictive for production)
        if not (1 <= min_zoom <= 18) or not (1 <= max_zoom <= 18) or min_zoom > max_zoom:
            return jsonify({"error": "Invalid zoom levels (max 18 for production)"}), 400
        
        # Validate buffer
        if not (0.001 <= buffer <= 0.05):  # Reduced max buffer for production
            return jsonify({"error": "Buffer must be between 0.001 and 0.05 degrees"}), 400
        ...
        # Limit tile count for Railway (prevent resource exhaustion)
        if total_tiles_estimate > 10000:
            return jsonify({
                "error": f"Too many tiles requested ({total_tiles_estimate}). Please reduce zoom levels or buffer size."
            }), 400
...
def cleanup_session(session_id):
    ...
    try:
        if session_id in progress_data:
            ...
            # Optionally clean up the file (uncomment if needed)
            # if output_file and os.path.exists(output_file):
            #     os.remove(output_file)
...
def health_check():
    ...
    return jsonify({
        ...
        "version": "2.0",
        "platform": os.name,
        "qgis_available": QGIS_PATH is not None
    })
...
if __name__ == "__main__":
    ...
    if QGIS_PATH and os.path.exists(QGIS_PATH):
        ...
        print(f"  ⚠ QGIS not found (manual download still available)")
